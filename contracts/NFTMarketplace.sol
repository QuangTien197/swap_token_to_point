...
contract NFTMarketplace is ERC721URIStorage, IERC721Receiver {
    ...
    constructor(address _token) ERC721("T-REX", "TREX") {
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (
                ...
                idToMarketItem[i + 1].list == true &&
                idToMarketItem[i + 1].bid == false
            ) {
    ...
    function fetchSellItemsListed(
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToMarketItem[i + 1].seller == _sell &&
                idToMarketItem[i + 1].bid == false &&
                idToMarketItem[i + 1].list == true
            ) {
                itemCount += 1;
    ...
    function fetchMarketItemsAuction()
        public
        view
        returns (MarketItem[] memory)
    {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToMarketItem[i + 1].owner == address(this) &&
                idToMarketItem[i + 1].bid == true &&
                idToMarketItem[i + 1].list == false
            ) {
                itemCount += 1;
            }
        }
    ...
    function fetchSellItemsAuction(
        address _sell
    ) public view returns (MarketItem[] memory) {
        uint256 totalItemCount = _tokenIds.current();
        uint256 itemCount = 0;
    ...
    function createAuction(
        ...
        safeTransferFrom(msg.sender, payable(address(this)), _tokenId);
    ...
    function joinAuction(uint256 _auctionId, uint256 _bid) public {
        ...
        require(block.timestamp >= _auction.startAt, "Auction has not started");
        ...
        uint256 _minBid = _auction.lastBidder == address(0)
            ...
            : (_auction.lastBid * discountRate) / 100;
    ...
    function finishAuction(
        ...
        //     auction[_auctionId].startingPrice;
        ...
        payable(auction[_auctionId].seller).transfer(lastBid);
        // transfer(auction[_auctionId].auctioneer, auctioneerReceive);
    ...
    modifier onlyAuctioneer(uint256 _auctionId) {
        require(
            msg.sender == auction[_auctionId].seller,
